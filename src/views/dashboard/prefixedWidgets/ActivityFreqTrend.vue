...
import { ActivityFreqTrendData } from './SampleData';

@Component({})
export default class ActivityFreqTrend extends Mixins(WidgetMixin, UtilsMixin) {
  /* Prop 선언 */

  /* mapGetters 변수 선언 */

  /* 클래스 변수 선언 */
  chart: BasicLineChart | BasicBarChart = null;

  // 액티비티 추이
  name: string = 'activity.chart.trend';
  ...
  data: IWidgetDataConfiguration = {
    ...
    dataParams: {
      ...
      nLimit: 10, // optional, 액티비티 갯수
    },
  };

  sampleData = ActivityFreqTrendData;
  ...
  dataProcessing(res: any) {
    ...
    if (!this.isNoData) {
      ...
      this.chart.makeXAxisSeries(data.data);
    ...
    const option: EChartOption = {
      ...
      tooltip: {
        ...
        formatter: (params: any | any[]) => {
          const string = [`${params[0].axisValue}${this.data.dataParams.timeFormat === 'YYYY-MM-DD HH24' ?this.$t('view.dashboard.activityTrend.HH'):'' }`];
          for (const item of params) {
            string.push(`${item.marker} ${item.seriesName} : ${this.numberWithCommas(item.data[1])}`);
          }
          return string.join('<br>');
        },
      },
      grid: theme.countGrid.twoBottomContent,
      legend: {
        show: true,
        type: 'scroll',
