      ...
      <div class="chart-list-component">
        <div class="row chart-list-content">
          ...
          <div class="chart-content grid-content col-xs-6 col-md-6 h-2x-chart">
            <cost-ratio-by-activity></cost-ratio-by-activity>
          </div>
          <div class="chart-content grid-content col-xs-6 col-md-6">
            <cost-mean-resource></cost-mean-resource>
            <cost-mean-originator style="margin-top: 5px !important"></cost-mean-originator>
          </div>
        </div>
      </div>
    </div>
...
export default class DashboardCostTab extends Mixins(DashboardTabMixin) {
  /* Prop 선언 */
  /* mapGetters 변수 선언 */
  /* 클래스 변수 선언 */
  // 총 비용, 케이스 별 최소, 평균, 최대 비용
  kpis: IKeyValueObject = {
    'kpi-01-total': { id: 'kpi-01-total', name: 'cost.kpi.total', value: '0', valueFormat: 'number', dataParams: { durUnit: 'month', durValue: 1 } },
    'kpi-02-min': { id: 'kpi-02-min', name: 'cost.kpi.min', value: '0', valueFormat: 'number', dataParams: { durUnit: 'month', durValue: 1 } },
    'kpi-03-avg': { id: 'kpi-03-avg', name: 'cost.kpi.avg', value: '0', valueFormat: 'number', dataParams: { durUnit: 'month', durValue: 1 } },
    'kpi-04-max': { id: 'kpi-04-max', name: 'cost.kpi.max', value: '0', valueFormat: 'number', dataParams: { durUnit: 'month', durValue: 1 } },
  };
  /* refs 선언 */
  /* Watch 선언 */
  /* navigator guard 선언 */
  ...
  mounted() {
    EventBus.$on('getWidgetData-CostTab', () => {
      for (const child of this.$children) {
        const widget = child as WidgetMixin;
        const actionInfos = this.getActionInfos();
        const action = actionInfos ? actionInfos[`cost-${widget.name}`] : null;
  ...
  beforeDestroy() {
    EventBus.$off('getWidgetData-CostTab');
  }
  /* 메소드 선언 */
  workflowActionBuilder(client: IClient): { [key: string]: IWorkFlowRequestAction } {
    ...
    for (const child of this.$children) {
      const widget = child as WidgetMixin;
      const workflowAction = widget.workflowActionBuilder(client, 'cost');
      if (workflowAction !== null) {
        actions[`cost-${widget.name}`] = workflowAction;
      }
    }
