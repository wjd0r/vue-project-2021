  ...
  <div class="right-table-component rework-table-component">
    <div class="right-table-content rework-table-content">
      <div class="content-header">
        ...
        <span class="count-text">({{ numberWithCommas(rowData.totalRows) }}) {{ $t('view.process.table.bottlenecksBase') }}</span>
...
export default class ProcessBottleneck extends Mixins(AnalysisMixin, UtilsMixin) {
  rowData: {
  ...
  getBottleneckTable() {
    ...
    // service/run/process.discovery.model.fhm.pf.outliers
    this.$store
      .dispatch('process/getBottleneck', data)
  ...
  onClickBottleneckTableCSV() {
    ...
    const data = this.requestBuilder();
    data[0].size = -1;
    // service/download/process.discovery.model.fhm.pf.outliers.csv
    this.$store.dispatch('process/getBottleneckCSV', data).then((res: AxiosResponse) => {
      // 파일 제목 생성
      let title = `${this.$route.params.projectId === 'iprdemo' ? this.$t('view.' + this.eventView.name) : this.eventView.name}_${this.$t('view.process.table.bottlenecksTitleDiscovery')}`;
      // 기간
  ...
  workflowActionBuilder(client: IClient): IWorkFlowRequestAction {
    return {
      _type: 'ServiceAction',
      id: 'bottleneck',
      serviceId: 'process.discovery.model.fhm.pf.outliers',
      client: {
        ...client,
      },
  ...
  // 그룹 cell value에 i18n 적용
  getGroupColumnName(value: string) {
    return this.$t('view.process.table.group') + value;
  }
    ...
    this.columnDefs = [
      {
        // 그룹
        headerValueGetter: () => {
          return this.$t('view.process.table.group');
        },
        field: 'group',
        colId: 'group',
        comparator: numberSort,
        valueFormatter: (params: ValueFormatterParams) => {
          return this.getGroupColumnName(params.data.group);
        },
        cellStyle: {
          'text-align': 'right',
        },
      ...
      },
      {
        // 도착 액티비티
        headerValueGetter: () => {
          return this.$t('view.process.table.target.discovery');
        },
        field: 'target',
        colId: 'target.discovery',
      },
      {
        // 병목기준
        headerValueGetter: () => {
      ...
      // { headerName: '구분', field: 'flowType' },
      {
        // 병목비율 (건수)
        headerValueGetter: () => {
          return this.$t('view.process.table.outlierCountCasesRatio');
        },
        field: 'outlierCountCasesRatio',
        colId: 'outlierCountCasesRatio',
        valueFormatter: (params: any) => {
          return (params.data.outlierCountCasesRatio * 100).toFixed(2) + '%';
        },
        cellStyle: {
          'text-align': 'right',
        },
      },
      {
        // 병목 케이스 평균
        headerValueGetter: () => {
          return this.$t('view.process.table.outlierAverageCases');
        },
        field: 'outlierAverageCases',
        colId: 'outlierAverageCases',
        valueFormatter: (params: ValueFormatterParams) => {
          return this.toDDHHMMSS(params.data.outlierAverageCases);
        },
        cellStyle: {
          'text-align': 'right',
        },
      },
      {
        // 케이스 평균
        headerValueGetter: () => {
          return this.$t('view.process.table.averageCases');
        },
        field: 'averageCases',
        colId: 'averageCases',
        valueFormatter: (params: ValueFormatterParams) => {
          return this.toDDHHMMSS(params.data.averageCases);
        },
        cellStyle: {
          'text-align': 'right',
        },
      },
      {
        // 병목비율 (시간)
        headerValueGetter: () => {
          return this.$t('view.process.table.outlierAverageCasesRatio');
        },
        field: 'outlierAverageCasesRatio',
        colId: 'outlierAverageCasesRatio',
        valueFormatter: (params: ValueFormatterParams) => {
          return (params.data.outlierAverageCasesRatio * 100).toFixed(2) + '%';
        },
        cellStyle: {
          'text-align': 'right',
        },
