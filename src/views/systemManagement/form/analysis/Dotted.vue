<template>
  <div class="form-content">
    ...
    <div class="content-body">
      <b-form-group :label="$t('view.setting.label.yaxis')" label-cols="2">
        <b-form-select v-model="setting.dotted.component" :disabled="isMobileBlock()">
          <template v-for="item, index in dotted.component">
            <b-form-select-option :value="item.value" :key="index">{{ $t(`view.setting.selectbox.${item.text}`) }}</b-form-select-option>
          </template>
        </b-form-select>
      </b-form-group>
      <b-form-group :label="$t('view.setting.label.classificationAttribute')" label-cols="2">
        <b-form-select v-model="setting.dotted.value" :disabled="isMobileBlock()">
          <template v-for="item, index in dotted.value">
            <b-form-select-option :value="item.value" :key="index">{{ $t(`view.setting.selectbox.${item.text}`) }}</b-form-select-option>
          </template>
        </b-form-select>
      </b-form-group>
...
export default class Dotted extends Mixins(UtilsMixin) {
  ...
  created() {
    this.onReset();
    this.dotted = JSON.parse(JSON.stringify($options)).default.analysis.dotted;
  }
