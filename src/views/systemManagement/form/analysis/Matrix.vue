<template>
  <div class="form-content">
    ...
    <div class="content-body">
      <b-form-group :label="$t('view.setting.matrix.howToView')" label-cols="2">
        <b-form-select v-model="setting.matrix.selectedX" :disabled="isMobileBlock()">
          <template v-for="item, index in matrix.selectedX">
            <b-form-select-option :value="item.value" :key="index">{{ $t(`view.setting.selectbox.${item.text}`) }}</b-form-select-option>
          </template>
        </b-form-select>
      </b-form-group>
      <b-form-group :label="$t('view.setting.matrix.conditionCriteria')" label-cols="2">
        <b-form-select v-model="setting.matrix.selectedY" :disabled="isMobileBlock()">
          <template v-for="item, index in matrix.selectedY">
            <b-form-select-option :value="item.value" :key="index">{{ $t(`view.setting.selectbox.${item.text}`) }}</b-form-select-option>
          </template>
        </b-form-select>
      </b-form-group>
      <b-form-group :label="$t('view.setting.matrix.valuetype')" label-cols="2">
        <b-form-select v-model="setting.matrix.selectedColorBy" :disabled="isMobileBlock()">
          <template v-for="item, index in matrix.selectedColorBy">
            <b-form-select-option :value="item.value" :key="index">{{ $t(`view.setting.selectbox.${item.text}`) }}</b-form-select-option>
          </template>
        </b-form-select>
      </b-form-group>
...
export default class Matrix extends Mixins(UtilsMixin) {
  ...
  created() {
    this.onReset();
    this.matrix = JSON.parse(JSON.stringify($options)).default.analysis.matrix;
  }
